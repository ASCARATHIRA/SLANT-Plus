...
def test_network():
    ...
    for i in hours:
        ...
        for u in range(N):
	        ...
	        del_m = [0.0] * N
                for j in range(N):
                    del_m[j] = abs(m-get_sent_2(j,G,t_last))
        ...
        if num > 0:
            ...
            pol_list.append(sum_p / float(num)) 
        else:
            mse_list.append(sum)
            pol_list.append(sum_p)
    '''
    pol_list = []
    ind = 0
    ...
    for i in hours:
        ...
        for u in range(N):
            ...
            g1 = g
            print type(G[u]), G[u]
	    meanG = sum([i[0] for i in G[u]])/len(G[u])
            if ind < len(t_opinions):
               g1 = t_opinions[ind][u]
            if g > -1 and (g-meanG) * (g1-meanG) < 0:
               sum += 1.0 
            if g > -1: 
               num += 1
        if num > 0:
