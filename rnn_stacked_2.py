...
def rnn_cell(u,t,m,state,state_x,state2,state2_x,state3,state3_x): # modify weights
    with tf.variable_scope('rnn_cell', reuse=True):
        ...
        lamb = tf.exp(w9 + w10 * t + w11 * h3) # depends on last layer hidden state h3
        ...
        x = tf.tanh(w9_x + w11_x * h3_x)

        return h, lamb, h_x, x, h2, h2_x, h3, h3_x
...
for i in range(batch_size):
    state, lamb, state_x, x, st2, st2_x, st3, st3_x = rnn_cell(tf.gather(U,i),tf.gather(T,i),tf.gather(M,i),state,state_x,st2,st2_x,st3,st3_x)
    lambdas.append(lamb)
...
losses = get_integral(lamb_high_states) - log_lambda(lambdas) + get_mse(opinions)
total_loss = losses
...
def train_network(num_epochs,state_size=N, verbose=True):
    with tf.Session() as sess:
        import os
        #if not os.path.isdir(folder+'logs_2_stacked'+str(N)):
            #os.mkdir(folder+'logs_2_stacked'+str(N))
        #train_writer = tf.train.SummaryWriter(folder+'logs_2_stacked'+str(N), sess.graph)
        sess.run(tf.initialize_all_variables())
        training_losses = []
        if os.path.isfile(folder+"model_2_stacked"+str(N)+".ckpt"):
            saver.restore(sess, folder+"model_2_stacked"+str(N)+".ckpt")
            training_state = np.zeros((state_size))
            t_w1, t_w2, t_w3, t_w4, t_w5, t_w6, t_w7, t_w8, t_w9, t_w10, t_w11, t_w12, t_w13, t_w14, t_w22, t_w23, t_w24, t_W = sess.run(
                [w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12,w13,w14,w22,w23,w24,W],feed_dict={U:user,T:dt,M:dm,init_state:training_state,O:X,ADJ:adj})
            t_w1_x, t_w2_x, t_w3_x, t_w4_x, t_w5_x, t_w6_x, t_w7_x, t_w8_x, t_w9_x, t_w10_x, t_w11_x, t_w12_x, t_w13_x, t_w14_x, t_w22_x, t_w23_x, t_w24_x, t_W_x = sess.run(
                [w1_x,w2_x,w3_x,w4_x,w5_x,w6_x,w7_x,w8_x,w9_x,w10_x,w11_x,w12_x,w13_x,w14_x,w22_x,w23_x,w24_x,W_x],feed_dict={U:user,T:dt,M:dm,init_state:training_state,O:X,ADJ:adj})
        else:
            epoch = 0
            ...
            while epoch < num_epochs:
                training_state = np.zeros((state_size))
                if verbose:
                    training_loss_, _ = sess.run([total_loss,train_step],feed_dict={U:user,T:dt,M:dm,init_state:training_state,O:X,ADJ:adj})
            ...
            t_w1_x, t_w2_x, t_w3_x, t_w4_x, t_w5_x, t_w6_x, t_w7_x, t_w8_x, t_w9_x, t_w10_x, t_w11_x, t_w12_x, t_w13_x, t_w14_x, t_w22_x, t_w23_x, t_w24_x, t_W_x = sess.run(
                [w1_x,w2_x,w3_x,w4_x,w5_x,w6_x,w7_x,w8_x,w9_x,w10_x,w11_x,w12_x,w13_x,w14_x,w22_x,w23_x,w24_x,W_x],feed_dict={U:user,T:dt,M:dm,init_state:training_state,O:X,ADJ:adj})
            saver.save(sess, folder+"model_2_stacked"+str(N)+".ckpt")
    return training_losses,t_w1, t_w2, t_w3, t_w4, t_w5, t_w6, t_w7, t_w8, t_w9, t_w10, t_w11, t_w12, t_w13, t_w14, t_w22, t_w23, t_w24, t_W, t_w1_x, t_w2_x, t_w3_x, t_w4_x, t_w5_x, t_w6_x, t_w7_x, t_w8_x, t_w9_x, t_w10_x, t_w11_x, t_w12_x, t_w13_x, t_w14_x, t_w22_x, t_w23_x, t_w24_x, t_W_x

training_losses,t_w1, t_w2, t_w3, t_w4, t_w5, t_w6, t_w7, t_w8, t_w9, t_w10, t_w11, t_w12, t_w13, t_w14, t_w22, t_w23, t_w24, t_W, t_w1_x, t_w2_x, t_w3_x, t_w4_x, t_w5_x, t_w6_x, t_w7_x, t_w8_x, t_w9_x, t_w10_x, t_w11_x, t_w12_x, t_w13_x, t_w14_x, t_w22_x, t_w23_x, t_w24_x, t_W_x = train_network(2000)
...
def rnn_cell_test(u,t,m,state,state_x,state2,state2_x,state3,state3_x):
    ...
    lamb = np.exp(t_w9 + t_w10 * t + t_w11 * h3)
...
def test_network():
    ...
    t_op_high_states = []
    for i in range(batch_size,total_size):
        t_state, t_lamb, t_state_x, t_x, t_st2, t_st2_x, t_st3, t_st3_x = rnn_cell_test(user[i],dt[i],dm[i],t_state,t_state_x,t_st2,t_st2_x,t_st3,t_st3_x)
    ...
    lamb_loss = t_get_integral(t_lamb_high_states) - t_log_lambda(t_lambdas)
