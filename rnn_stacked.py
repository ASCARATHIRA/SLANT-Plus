...
def rnn_cell(u,t,m,state,state_x,state2,state2_x): # modify weights
    with tf.variable_scope('rnn_cell', reuse=True):
        ...
        h2 = tf.tanh(w12 * h + w13 * state2 + w14) # w14 is bias term introdeced
        lamb = tf.exp(w9 + w10 * t + w11 * h2) # depends on last layer hidden state h2
        ...
        x = tf.tanh(w9_x + w11_x * h2_x)

        return h, lamb, h_x, x, h2, h2_x
...
for i in range(batch_size):
    state, lamb, state_x, x, st2, st2_x = rnn_cell(tf.gather(U,i),tf.gather(T,i),tf.gather(M,i),state,state_x,st2,st2_x)
    lambdas.append(lamb)
    lamb_states.append(state)
    opinions.append(x)
    op_states.append(state_x)
    lamb_high_states.append(st2)
    op_high_states.append(st2_x)
...
def train_network(num_epochs,state_size=N, verbose=True):
    with tf.Session() as sess:
        import os
        #if not os.path.isdir(folder+'logs_stacked'+str(N)):
            #os.mkdir(folder+'logs_stacked'+str(N))
        #train_writer = tf.train.SummaryWriter(folder+'logs_stacked'+str(N), sess.graph)
        sess.run(tf.initialize_all_variables())
        training_losses = []
        if os.path.isfile(folder+"model_stacked"+str(N)+".ckpt"):
            saver.restore(sess, folder+"model_stacked"+str(N)+".ckpt")
            training_state = np.zeros((state_size))
            t_w1, t_w2, t_w3, t_w4, t_w5, t_w6, t_w7, t_w8, t_w9, t_w10, t_w11, t_w12, t_w13, t_w14, t_W = sess.run(
                [w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12,w13,w14,W],feed_dict={U:user,T:dt,M:dm,init_state:training_state,O:X,ADJ:adj})
            t_w1_x, t_w2_x, t_w3_x, t_w4_x, t_w5_x, t_w6_x, t_w7_x, t_w8_x, t_w9_x, t_w10_x, t_w11_x, t_w12_x, t_w13_x, t_w14_x, t_W_x = sess.run(
                [w1_x,w2_x,w3_x,w4_x,w5_x,w6_x,w7_x,w8_x,w9_x,w10_x,w11_x,w12_x,w13_x,w14_x,W_x],feed_dict={U:user,T:dt,M:dm,init_state:training_state,O:X,ADJ:adj})
        else:
            epoch = 0
            ...
            t_w1_x, t_w2_x, t_w3_x, t_w4_x, t_w5_x, t_w6_x, t_w7_x, t_w8_x, t_w9_x, t_w10_x, t_w11_x, t_w12_x, t_w13_x, t_w14_x, t_W_x = sess.run(
                [w1_x,w2_x,w3_x,w4_x,w5_x,w6_x,w7_x,w8_x,w9_x,w10_x,w11_x,w12_x,w13_x,w14_x,W_x],feed_dict={U:user,T:dt,M:dm,init_state:training_state,O:X,ADJ:adj})
            saver.save(sess, folder+"model_stacked"+str(N)+".ckpt")
    return training_losses,t_w1, t_w2, t_w3, t_w4, t_w5, t_w6, t_w7, t_w8, t_w9, t_w10, t_w11, t_w12, t_w13, t_w14, t_W, t_w1_x, t_w2_x, t_w3_x, t_w4_x, t_w5_x, t_w6_x, t_w7_x, t_w8_x, t_w9_x, t_w10_x, t_w11_x, t_w12_x, t_w13_x, t_w14_x, t_W_x

training_losses,t_w1, t_w2, t_w3, t_w4, t_w5, t_w6, t_w7, t_w8, t_w9, t_w10, t_w11, t_w12, t_w13, t_w14, t_W, t_w1_x, t_w2_x, t_w3_x, t_w4_x, t_w5_x, t_w6_x, t_w7_x, t_w8_x, t_w9_x, t_w10_x, t_w11_x, t_w12_x, t_w13_x, t_w14_x, t_W_x = train_network(2000)
...
def rnn_cell_test(u,t,m,state,state_x,state2,state2_x):
    ...
    lamb = np.exp(t_w9 + t_w10 * t + t_w11 * h2)
...
def test_network():
    ...
    t_op_high_states = []
    for i in range(batch_size,total_size):
        t_state, t_lamb, t_state_x, t_x, t_st2, t_st2_x = rnn_cell_test(user[i],dt[i],dm[i],t_state,t_state_x,t_st2,t_st2_x)
        t_lambdas.append(t_lamb)
        t_lamb_states.append(t_state)
        t_opinions.append(t_x)
        t_op_states.append(t_state_x)
        t_lamb_high_states.append(t_st2)
        t_op_high_states.append(t_st2_x)
