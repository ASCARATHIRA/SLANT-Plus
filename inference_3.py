...
def test_network():
    ...
    for i in range(l,r):
        if H[i][0]==u:
            tempx.append(i-l+1)
            expected_op.append(H[i][2])
        elif adj[H[i][0]][u]==1:
            tempx2.append(i-l+1)
            nbr_op.append(H[i][2])
    '''
    print N,'\n',len(t_op_states)
    print_rowwise(t_op_states)
    print_rowwise(t_op_low_states)
    print_rowwise(t_op_high_states)
    print_rowwise(t_op_highest_states)
    #plt.plot(tempx,expected_op,'ro')
    #plt.plot(tempx2,nbr_op,'bs')
    '''
    h0list = [i[u] for i in t_op_states]
    h1list = [i[u] for i in t_op_low_states]
    h2list = [i[u] for i in t_op_high_states]
    h3list = [i[u] for i in t_op_highest_states]
    xlist = [i[u] for i in t_opinions]
    X = [i+1-l for i in range(l,r)]
    print 'X = ', X, '\nh0 = ', h0list, '\nh1 = ', h1list, '\nh2 = ', h2list, '\nh3 = ', h3list
    # plt.plot(X[1:],xlist[1:],linewidth=2.0,color='r')
    ...
    # plt.plot(X,h3list,'b')
    # plt.plot(X,xlist,'y')
    #plt.legend(['expressed opinions','neighbor opinion'], loc='lower right')
    # plt.ylabel('Internal state values')
