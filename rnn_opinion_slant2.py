...
def rnn_cell(u,t,m,state,state_x): # modify weights
    with tf.variable_scope('rnn_cell', reuse=True):
        ...
        h = tf.sigmoid(w1 * tf.exp(-w2 * t) * state + tf.matmul([u], W)[0,:] * tf.matmul([u], ADJ)[0,:] * m)
        lamb = tf.exp(w9 + w10 * t + w11 * h)

        #sig1_x = w3_x * tf.sigmoid(w4_x * (m - w5_x))
        #sig2_x = w6_x * tf.sigmoid(-w7_x * (m - w8_x))
        h_x = tf.sigmoid(w1_x * tf.exp(-w2_x * t) * state_x + tf.matmul([u], W_x)[0,:] * tf.matmul([u], ADJ)[0,:] * m)
...
def train_network(num_epochs,state_size=N, verbose=True):
    with tf.Session() as sess:
        ...
        training_losses = []
        if os.path.isfile(folder+"model_opinion_slant_"+str(t_s)+"_"+str(N)+".ckpt"):
            saver.restore(sess, folder+"model_opinion_slant_"+str(t_s)+"_"+str(N)+".ckpt")
            ...
            while epoch < num_epochs:
                ...
                if verbose:
                    ...
                    #shape,training_loss_, _ = sess.run([shapeT,total_loss,train_step],feed_dict={U:user,T:dt,M:dm,init_state:training_state})
            ...
            t_w1_x, t_w2_x, t_w3_x, t_w4_x, t_w5_x, t_w6_x, t_w7_x, t_w8_x, t_w9_x, t_w10_x, t_w11_x,t_W_x = sess.run(
                [w1_x,w2_x,w3_x,w4_x,w5_x,w6_x,w7_x,w8_x,w9_x,w10_x,w11_x,W_x],feed_dict={U:user,T:dt,M:dm,init_state:training_state,O:X,ADJ:adj})
            saver.save(sess, folder+"model_opinion_slant_"+str(t_s)+"_"+str(N)+".ckpt")
...
training_losses,t_w1, t_w2, t_w3, t_w4, t_w5, t_w6, t_w7, t_w8, t_w9, t_w10, t_w11, t_W,t_w1_x, t_w2_x, t_w3_x, t_w4_x, t_w5_x, t_w6_x, t_w7_x, t_w8_x, t_w9_x, t_w10_x, t_w11_x,t_W_x = train_network(200)
...
def rnn_cell_test(u,t,m,state,state_x):
    ...
    h = sigmoid(t_w1 * np.exp(-t_w2 * t) * state + np.matmul([u], t_W)[0,:] * np.matmul([u], adj)[0,:] * m)
    lamb = np.exp(t_w9 + t_w10 * t + t_w11 * h)

    #sig1_x = t_w3_x * sigmoid(t_w4_x * (m - t_w5_x))
    #sig2_x = t_w6_x * sigmoid(-t_w7_x * (m - t_w8_x))
    h_x = sigmoid(t_w1_x * np.exp(-t_w2_x * t) * state_x + np.matmul([u], t_W_x)[0,:] * np.matmul([u], adj)[0,:] * m)
