...
with tf.variable_scope('rnn_cell', reuse=None):
    ...
    w28 = tf.get_variable('w28', [state_size], initializer=tf.random_uniform_initializer(0.1, .1))
    w29 = tf.get_variable('w29', [state_size], initializer=tf.random_uniform_initializer(0.1, .1))
    w30 = tf.get_variable('w30', [state_size], initializer=tf.random_uniform_initializer(0.1, .1))
    w31 = tf.get_variable('w31', [state_size], initializer=tf.random_uniform_initializer(0.1, .1))
    w32 = tf.get_variable('w32', [state_size], initializer=tf.random_uniform_initializer(0.1, .1))
    w33 = tf.get_variable('w33', [state_size], initializer=tf.random_uniform_initializer(0.1, .1))
    ...
    w28_x = tf.get_variable('w28_x', [state_size], initializer=tf.random_uniform_initializer(0.1, .1))
    w29_x = tf.get_variable('w29_x', [state_size], initializer=tf.random_uniform_initializer(0.1, .1))
    w30_x = tf.get_variable('w30_x', [state_size], initializer=tf.random_uniform_initializer(0.1, .1))
    w31_x = tf.get_variable('w31_x', [state_size], initializer=tf.random_uniform_initializer(0.1, .1))
    w32_x = tf.get_variable('w32_x', [state_size], initializer=tf.random_uniform_initializer(0.1, .1))
    w33_x = tf.get_variable('w33_x', [state_size], initializer=tf.random_uniform_initializer(0.1, .1))
    W_x = tf.get_variable('W_x', [state_size, state_size], initializer=tf.random_uniform_initializer(0.0,1.0))


def rnn_cell(u,t,m,state,state_x,state2,state2_x,state3,state3_x,state4,state4_x,state5,state5_x,state6,state6_x): # modify weights
    with tf.variable_scope('rnn_cell', reuse=True):
        ...
        h5 = tf.tanh(w28 * h4 + w29 * state5 + w30)
        h6 = tf.tanh(w31 * h5 + w32 * state6 + w33)
        lamb = tf.exp(w9 + w10 * t + w11 * h6) # depends on last layer hidden state h3
        ...
        h5_x = tf.tanh(w28_x * h4_x + w29_x * state5_x + w30_x)
        h6_x = tf.tanh(w31_x * h5_x + w32_x * state6_x + w33_x)
        x = tf.tanh(w9_x + w11_x * h6_x)

        return h, lamb, h_x, x, h2, h2_x, h3, h3_x, h4, h4_x, h5, h5_x, h6, h6_x
...
for i in range(size):
    state, lamb, state_x, x, st2, st2_x, st3, st3_x, st4, st4_x, st5, st5_x, st6, st6_x = rnn_cell(tf.gather(U,i),tf.gather(T,i),tf.gather(M,i),state,state_x,st2,st2_x,st3,st3_x,st4,st4_x,st5,st5_x,st6,st6_x)
    lambdas.append(lamb)
    ...
    lamb_5_states.append(st5)
    op_5_states.append(st5_x)
    lamb_6_states.append(st6)
    op_6_states.append(st6_x)
...
losses = get_integral(lamb_6_states) - log_lambda(lambdas) + get_mse(opinions)
total_loss = losses
...
t_w28 = np.zeros((state_size),dtype=np.float32)
t_w29 = np.zeros((state_size),dtype=np.float32)
t_w30 = np.zeros((state_size),dtype=np.float32)
t_w31 = np.zeros((state_size),dtype=np.float32)
t_w32 = np.zeros((state_size),dtype=np.float32)
t_w33 = np.zeros((state_size),dtype=np.float32)
...
t_w28_x = np.zeros((state_size),dtype=np.float32)
t_w29_x = np.zeros((state_size),dtype=np.float32)
t_w30_x = np.zeros((state_size),dtype=np.float32)
t_w31_x = np.zeros((state_size),dtype=np.float32)
t_w32_x = np.zeros((state_size),dtype=np.float32)
t_w33_x = np.zeros((state_size),dtype=np.float32)
t_W_x = np.zeros((state_size,state_size),dtype=np.float32)

def train_network(num_epochs,state_size=N, verbose=True):
    with tf.Session() as sess:
        import os
        #if not os.path.isdir(folder+'logs_5_stacked'+str(N)):
            #os.mkdir(folder+'logs_5_stacked'+str(N))
        #train_writer = tf.train.SummaryWriter(folder+'logs_5_stacked'+str(N), sess.graph)
        sess.run(tf.initialize_all_variables())
        training_losses = []
        if os.path.isfile(folder+"model_5_stacked"+str(N)+".ckpt"):
            saver.restore(sess, folder+"model_5_stacked"+str(N)+".ckpt")
            training_state = np.zeros((state_size))
            t_w1, t_w2, t_w3, t_w4, t_w5, t_w6, t_w7, t_w8, t_w9, t_w10, t_w11, t_w12, t_w13, t_w14, t_w22, t_w23, t_w24, t_w25, t_w26, t_w27, t_w28, t_w29, t_w30,t_w31, t_w32, t_w33, t_W = sess.run(
                [w1,w2,w3,w4,w5,w6,w7,w8,w9,w10,w11,w12,w13,w14,w22,w23,w24,w25,w26,w27,w28,w29,w30,w31,w32,w33,W])
            t_w1_x, t_w2_x, t_w3_x, t_w4_x, t_w5_x, t_w6_x, t_w7_x, t_w8_x, t_w9_x, t_w10_x, t_w11_x, t_w12_x, t_w13_x, t_w14_x, t_w22_x, t_w23_x, t_w24_x, t_w25_x, t_w26_x, t_w27_x, t_w28_x, t_w29_x, t_w30_x, t_w31_x, t_w32_x, t_w33_x, t_W_x = sess.run(
                [w1_x,w2_x,w3_x,w4_x,w5_x,w6_x,w7_x,w8_x,w9_x,w10_x,w11_x,w12_x,w13_x,w14_x,w22_x,w23_x,w24_x,w25_x,w26_x,w27_x,w28_x,w29_x,w30_x,w31_x,w32_x,w33_x,W_x])
        else:
            epoch = 0
            e = 0
            ...
            t_w1_x, t_w2_x, t_w3_x, t_w4_x, t_w5_x, t_w6_x, t_w7_x, t_w8_x, t_w9_x, t_w10_x, t_w11_x, t_w12_x, t_w13_x, t_w14_x, t_w22_x, t_w23_x, t_w24_x, t_w25_x, t_w26_x, t_w27_x, t_w28_x, t_w29_x, t_w30_x, t_w31_x, t_w32_x, t_w33_x, t_W_x = sess.run(
                [w1_x,w2_x,w3_x,w4_x,w5_x,w6_x,w7_x,w8_x,w9_x,w10_x,w11_x,w12_x,w13_x,w14_x,w22_x,w23_x,w24_x,w25_x,w26_x,w27_x,w28_x,w29_x,w30_x,w31_x,w32_x,w33_x,W_x])
            saver.save(sess, folder+"model_5_stacked"+str(N)+".ckpt")
    return training_losses,t_w1, t_w2, t_w3, t_w4, t_w5, t_w6, t_w7, t_w8, t_w9, t_w10, t_w11, t_w12, t_w13, t_w14, t_w22, t_w23, t_w24, t_w25, t_w26, t_w27,t_w28,t_w29,t_w30,t_w31,t_w32,t_w33, t_W, t_w1_x, t_w2_x, t_w3_x, t_w4_x, t_w5_x, t_w6_x, t_w7_x, t_w8_x, t_w9_x, t_w10_x, t_w11_x, t_w12_x, t_w13_x, t_w14_x, t_w22_x, t_w23_x, t_w24_x, t_w25_x, t_w26_x, t_w27_x,t_w28_x,t_w29_x,t_w30_x,t_w31_x,t_w32_x,t_w33_x, t_W_x

training_losses,t_w1, t_w2, t_w3, t_w4, t_w5, t_w6, t_w7, t_w8, t_w9, t_w10, t_w11, t_w12, t_w13, t_w14, t_w22, t_w23, t_w24, t_w25, t_w26, t_w27,t_w28,t_w29,t_w30,t_w31,t_w32,t_w33, t_W, t_w1_x, t_w2_x, t_w3_x, t_w4_x, t_w5_x, t_w6_x, t_w7_x, t_w8_x, t_w9_x, t_w10_x, t_w11_x, t_w12_x, t_w13_x, t_w14_x, t_w22_x, t_w23_x, t_w24_x, t_w25_x, t_w26_x, t_w27_x,t_w28_x,t_w29_x,t_w30_x,t_w31_x,t_w32_x,t_w33_x, t_W_x = train_network(2000)
...
def rnn_cell_test(u,t,m,state,state_x,state2,state2_x,state3,state3_x,state4,state4_x,state5,state5_x,state6,state6_x):
    ...
    h5 = np.tanh(t_w28 * h4 + t_w29 * state5 + t_w30)
    h6 = np.tanh(t_w31 * h5 + t_w32 * state6 + t_w33)
    lamb = np.exp(t_w9 + t_w10 * t + t_w11 * h6)
    ...
    h5_x = np.tanh(t_w28_x * h4_x + t_w29_x * state5_x + t_w30_x)
    h6_x = np.tanh(t_w31_x * h5_x + t_w32_x * state6_x + t_w33_x)
    x = np.tanh(t_w9_x + t_w11_x * h6_x)
...
def test_network():
    ...
    t_st5 = np.zeros((state_size))
    t_st5_x = np.zeros((state_size))
    t_st6 = np.zeros((state_size))
    t_st6_x = np.zeros((state_size))
    t_lamb_low_states = []
    t_op_low_states = []
    t_lamb_high_states = []
    t_op_high_states = []
    ...
    t_op_6_states = []
    for i in range(batch_size,total_size):
        t_state, t_lamb, t_state_x, t_x, t_st2, t_st2_x, t_st3, t_st3_x, t_st4, t_st4_x, t_st5, t_st5_x, t_st6, t_st6_x = rnn_cell_test(user[i],dt[i],dm[i],t_state,t_state_x,t_st2,t_st2_x,t_st3,t_st3_x,t_st4,t_st4_x,t_st5,t_st5_x,t_st6,t_st6_x)
        ...
        t_lamb_5_states.append(t_st5)
        t_op_5_states.append(t_st5_x)
        t_lamb_6_states.append(t_st6)
        t_op_6_states.append(t_st6_x)
    ...
    lamb_loss = t_get_integral(t_lamb_6_states) - t_log_lambda(t_lambdas)
