...
def test_network():
    ...
    co = open(folder+cName+'.txt','r')
    community=[]
    for line in co:
        user_in_community = [int(a) for a in line.split(',')]
        community.append(user_in_community)
    #print community
    comm = [-1 for i in range(N)]
    for u in range(N):
        for i in range(len(community)):
            if u in community[i]:
                comm[u] = i
                break
    c_op = [0.0 for i in range(len(community))]
    c_op_1 = [0.0 for i in range(len(community))]
    cnt = [0 for i in range(len(community))]
    hours = range(11)
    ...
    for i in hours:
        sum = 0.0
        tm = t_last + i * 3600
        c_op = [0.0 for k in range(len(community))]
        c_op_1 = [0.0 for k in range(len(community))]
        c_fr = [0.0 for k in range(len(community))]
        cnt = [0 for k in range(len(community))]
        ...
        for j in range(len(community)):
            if cnt[j] > 0 :
                ...
                sum+= (g-g1)**2
                fr = c_fr[j]/cnt[j] 
                if len(community[j]) not in mse_dict.keys():
                    mse_dict[len(community[j])]=[]
                mse_dict[len(community[j])].append((g-g1)**2)
                if len(community[j]) not in fr_dict.keys():
                    fr_dict[len(community[j])]=[]
                fr_dict[len(community[j])].append(fr) 
                #c_s_list.append((len(community[j]),(g-g1)**2))
        print sorted(mse_dict.keys())
        for m in sorted(mse_dict.keys()):
            lt = len(mse_dict[m])
            mse_dict[m] = np.sum(mse_dict[m])/lt
            c_s_mse.append(mse_dict[m])
        mse_list.append(sum/z)
        print c_s_mse
        print sorted(fr_dict.keys())
        for m in sorted(fr_dict.keys()):
            lt = len(fr_dict[m])
            fr_dict[m] = np.sum(fr_dict[m])/lt
            c_s_fr.append(fr_dict[m])
        print c_s_fr  
    pol_list = []
    # ind = 0
    # for i in hours:
